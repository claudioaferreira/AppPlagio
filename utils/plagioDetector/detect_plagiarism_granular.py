...
def get_mpnet_granular_model():
    ...
    if _mpnet_granular_model is None:
        print("Cargando modelo MPNet (Granular) en RAM... esto puede tardar un momento.")
        # Usamos el mismo modelo multilingüe
        _mpnet_granular_model = SentenceTransformer('paraphrase-multilingual-mpnet-base-v2') 
...
def find_plagiarism_with_roberta_segmentation(document_content):
    ...
    try:
        ...
        with pyodbc.connect(CNXN_STR) as cnxn:
            ...
            db_embeddings = []
            db_ids = []
            # Inicializamos embedding_np fuera para evitar error de referencia si el loop falla
            embedding_np = None 
        ...
        print(f"Error en find_plagiarism_with_roberta_segmentation: {e}")
        unload_mpnet_granular_model()
        raise e